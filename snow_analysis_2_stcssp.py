...
step2: process new modis data (STC-SSP) and swe with same projection and resolution

step:
- swe: xmrg -> tif -> reproject
- modis: clip -> resample (modis raw data is already RDHM projection)
...
for swe_xmrg_folder in swe_snowdate_folders:
    ...
    if os.path.isdir(swe_xmrg_folder):
        ...
        for time in snow_date.index:
            swe_xmrg_path = os.path.join(swe_xmrg_folder, snow_date[xmrg_col_name].ix[time])
            if os.path.isfile(swe_xmrg_path):
                ...
                if not os.path.isfile(swe_proj_path):
                    try:
                        ...
                        snow_date[proj_col_name].ix[time] = os.path.basename(swe_proj_path)
...
try:
    swe_proj_path = os.path.join(swe_proj_folder, snow_date[proj_col_name][0])  # provide a path that is the projected swe tif file
...
fail_modis_proj = []
for time in snow_date.index:
    modis_tif_path = os.path.join(modis_snowdate_folder, snow_date[tif_col_name].ix[time])
    if os.path.isfile(modis_tif_path):
        modis_clip_name = os.path.basename(modis_tif_path).replace('.tif', '_clip.tif')
        modis_clip_path = os.path.join(modis_proj_folder, modis_clip_name)

        # clip and resample the tif
        cmd_clip = "gdalwarp -overwrite -ts {} {} -cutline {} -crop_to_cutline -dstalpha {} {}".format(x_size, y_size, clipper_file_path, modis_tif_path, modis_clip_path)

        if not os.path.isfile(modis_clip_path):
            try:
                subprocess.call(shlex.split(cmd_clip))
                snow_date[proj_col_name].ix[time] = os.path.basename(modis_clip_path)
            except Exception as e:
