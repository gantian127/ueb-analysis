...
info:
- The modis binary is created by processing the new modis data (STC-SSP) where the snow cover value should divide 100
- The cloud pixel calculation is removed. Since the new data removed the cloud pixels
...
df_col_name = ['watershed', 'watershed_out', 'invalid',
            ...
            if os.path.isfile(swe_proj_path) and os.path.isfile(modis_proj_path):
                try:
                    ...
                    pixel_count.at[time, df_col_name] = [
                     count_watershed, count_watershed_out, count_invalid,
