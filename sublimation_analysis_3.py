...
for var in domain_ave_df.index:
    result = []
    var_array_sum_path = os.path.join(stats_folder, 'sum_{}.tif'.format(var))
    var_array_mean_path = os.path.join(stats_folder, 'annual_mean_{}.tif'.format(var))
    for tif_path in [var_array_sum_path, var_array_mean_path]:
        tif_array = gdalnumeric.LoadFile(tif_path)
        mask = (tif_array == -99999.0)
        mask_array = np.ma.array(tif_array, mask=mask)
