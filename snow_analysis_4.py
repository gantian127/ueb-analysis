...
step4:  use pixel count result to calculate stats and make analysis plots
...
for pixel_count_path in pixel_count_path_list:
    if os.path.isfile(pixel_count_path):
        ...
        valid_data = pixel_count[(pixel_count.weight >= 0.9) &
                                  ((pixel_count.modis_snow > 0) | (pixel_count.swe_snow > 0))]
...
def get_snow_stats(df):
    ...
    r2 = sum(pixel_count.weight * (pixel_count.modis_percent_snow - modis_ave) * (
        pixel_count.swe_percent_snow - swe_ave)) \
    ...
    return [modis_ave, swe_ave, mae, nse, r2]
...
for model in ['snow17', 'ueb']:
    ...
    if os.path.isfile(pixel_count_path):
        ...
        columns = ['modis ave', 'swe ave', 'mae', 'nse', 'r2', 'fitness']
        stats.append(pixel_count['fitness'].mean())

        with open(os.path.join(stats_folder, 'all_years_stats_{}.csv'.format(model)), "wb") as f:
            writer = csv.writer(f)
            writer.writerows([columns, stats])

        # plot: obs vs sim scatter
        fig, ax = plt.subplots()
        pixel_count.plot.scatter(x='modis_percent_snow', y='swe_percent_snow', ax=ax)
        ...
        ax.set_title('Comparison of percent snow of all years for {} model '.format(model))
        fig.savefig(os.path.join(stats_folder, 'all_years_percent_snow_scatter_{}'.format(model)))
...
print 'snow_analysis_3: stats calculation is done'
